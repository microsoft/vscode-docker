...
import { CommonRegistry, CommonTag, isDockerHubRegistryItem, isGenericV2RegistryItem } from '@microsoft/vscode-docker-registries';
...
export class DockerSiteCreateStep extends AzureWizardExecuteStep<IAppServiceContainerWizardContext> {
    ...
    private async getNewSiteConfig(context: IAppServiceContainerWizardContext): Promise<SiteConfig> {
        ...
        else if (isDockerHubRegistryItem(registryTI)) {
        ...
        else if (isGenericV2RegistryItem(registryTI)) {
            registryUrl = registryTI.baseUrl.toString();
        } else {
        ...
        const linuxFxVersion = `DOCKER|${getFullImageNameFromRegistryItem(this.tagItem)}`;

...
export async function logInToDockerCli(context: IActionContext, node?: RegistryTreeItemBase): Promise<void> {
    ...
    if (!username || !password) {
        ext.outputChannel.warn(localize('vscode-docker.commands.registries.logIn.skipping', 'WARNING: Skipping login for "{0}" because it does not require authentication.', creds.registryPath));
    } else {

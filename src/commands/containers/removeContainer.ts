...
export async function removeContainer(context: IActionContext, node?: ContainerTreeItem, nodes?: ContainerTreeItem[]): Promise<void> {
    nodes = await multiSelectNodes(
        { ...context, noItemFoundErrorMessage: l10n.t('No containers are available to remove') },
        ext.containersTree,
    ...
    if (nodes.length === 1) {
        confirmRemove = l10n.t('Are you sure you want to remove container "{0}"?', nodes[0].label);
    } else {
        confirmRemove = l10n.t('Are you sure you want to remove selected containers?');
    }

    // no need to check result - cancel will throw a UserCancelledError
    await context.ui.showWarningMessage(confirmRemove, { modal: true }, { title: l10n.t('Remove') });

    const removing: string = l10n.t('Removing container(s)...');

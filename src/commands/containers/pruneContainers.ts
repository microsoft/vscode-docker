...
export async function pruneContainers(context: IActionContext): Promise<void> {
    ...
    await vscode.window.withProgress(
        ...
        async () => {
            ...
            let message: string;
            if (result.containersDeleted?.length && result.spaceReclaimed) {
                ...
                message = localize('vscode-docker.commands.containers.prune.removed2', 'Removed stopped containers.', result.containersDeleted.length, convertToMB(result.spaceReclaimed));
            }

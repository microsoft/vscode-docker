...
export async function promptForPorts(ports: number[]): Promise<number[]> {
    let opt: vscode.InputBoxOptions = {
        ...
        prompt: localize('vscode-docker.commands.configUtils.whatPort', 'What port(s) does your app listen on? Enter a comma-separated list, or empty for no exposed port.'),
        value: ports.join(', '),
        validateInput: (value: string): string | undefined => {
            let result = splitPorts(value);
            if (!result) {
                return localize('vscode-docker.commands.configUtils.portsFormat', 'Ports must be a comma-separated list of positive integers (1 to 65535), or empty for no exposed port.');
            }
...
export async function quickPickPlatform(platforms?: Platform[]): Promise<Platform> {
    let opt: vscode.QuickPickOptions = {
        ...
        matchOnDetail: true,
        placeHolder: localize('vscode-docker.commands.configUtils.selectPlatform', 'Select Application Platform')
    }
...
export async function quickPickOS(): Promise<PlatformOS> {
    let opt: vscode.QuickPickOptions = {
        matchOnDescription: true,
        matchOnDetail: true,
        placeHolder: localize('vscode-docker.commands.configUtils.selectOS', 'Select Operating System')
    }
...
export async function quickPickGenerateComposeFiles(): Promise<boolean> {
    let opt: vscode.QuickPickOptions = {
        placeHolder: localize('vscode-docker.commands.configUtils.includeCompose', 'Include optional Docker Compose files?')
    }

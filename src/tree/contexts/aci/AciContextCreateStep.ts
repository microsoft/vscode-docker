...
export class AciContextCreateStep extends AzureWizardExecuteStep<IAciWizardContext> {
    ...
    public async execute(wizardContext: IAciWizardContext, progress: Progress<{ message?: string; increment?: number }>): Promise<void> {
        ...
        const command = `${dockerExePath()} context create aci ${wizardContext.contextName} --subscription-id ${wizardContext.subscriptionId} --resource-group ${wizardContext.resourceGroup.name}`;
        ...
        try {
            ...
            if (error.errorType === '5' || /not logged in/i.test(error.message)) {
                ...
                await executeAsTask(wizardContext, `${dockerExePath()} login azure --cloud-name ${wizardContext.environment.name}`, localize('vscode-docker.commands.contexts.create.aci.azureLogin', 'Azure Login'), { rejectOnError: true });
                await execAsync(command);
            } else {
                // Otherwise rethrow
                throw err;

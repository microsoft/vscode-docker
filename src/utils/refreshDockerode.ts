 ...
 */
export function refreshDockerode(): void {
    ...
    try {
        ...
        ext.dockerode = new Dockerode(getDockerodeOptions());
    } catch (error) {
...
function getDockerodeOptions(): DockerOptions | undefined {
...
function getSshAuthSock(): string | undefined {
    if (isWindows()) {
        ...
        const stdout = cp.execSync('ssh-agent -s', { timeout: 1000 }).toString();
...
function getDefaultDockerContext(): DockerOptions | undefined {

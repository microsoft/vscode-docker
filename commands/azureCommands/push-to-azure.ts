...
export async function pushAzure(context?: ImageNode): Promise<any> {
    ...
    if (imageToPush) {
        ...
        let cont = (err: any, stdout: any, stderr: any): void => {
            console.log(stdout);
            let jsonStdout = JSON.parse(stdout);
            let soughtsrvr: string = "";
            for (let currJsonStdout of jsonStdout) {
                let srvrName: string = currJsonStdout.acrLoginServer;
            ...
            let tagPrompts = async (): Promise<void> => {

...
export function keyNameFromKeyToken(keyToken:string): string {
	return keyToken.replace(/\:+$/g, '');
}

export function tokenValue(line:string, token:IToken): string {
	return line.substring(token.startIndex, token.endIndex);
}

export function tokensAtColumn(tokens:IToken[], charIndex:number): number[] {
 ...
 */
export function parseLine(line:string): IToken[] {
	var r: IToken[] = [];
...
export var RAW_KEY_INFO:{[keyName:string]:string;} = {
	'image': (
